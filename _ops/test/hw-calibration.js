 ...
 * 書き順       3% が吉から先に書く。口を4画で書く人 6%（縦の書き出しは、書くたびに角から線の太さほどずれる。writing() の注記）。
 ...
 * 口を4画で書く人（split11）は、横折の縦の書き出しを角から毎回ずらす（縦の向き・横とも SD は線の太さ＝一辺の 2.2%、
 * 線の太さの 2.5 倍で打ち切り）。指を置く位置は角ぴったりにはならないため。このずれの乱数は種から別に作り、
 * ほかの値の乱数（r）の並びは変えません（口を4画で書かない人の字は、このずれを入れる前とまったく同じ）。 */
const LWC = 0.022;
function writing(p, r, seed) {
  const e = () => gauss(r);
  const rs = mulberry32(((seed >>> 0) ^ 0x5117ab11) >>> 0), off = () => clamp(LWC * gauss(rs), -2.5 * LWC, 2.5 * LWC);
  const s11 = p.split11 ? { split11Start: off(), split11Side: off() } : {};
  return Object.assign({}, p, s11, {
    seed,
