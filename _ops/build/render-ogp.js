...
function template(card) {
  ...
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  .seal span { font-size:116px; font-weight:700; color:${accent}; letter-spacing:0; line-height:1; }
  ...
  .kicker {
    font-size:23px; letter-spacing:.34em; color:${accent};
  ...
  .sub {
    margin-top:26px; font-size:27px; line-height:1.74; letter-spacing:.04em;
    color:rgba(244,241,234,.78); white-space:pre-line; font-weight:400;
  }
  .rule { margin-top:32px; width:76px; height:3px; background:${accent}; }
  .foot {
    position:absolute; left:96px; bottom:62px;
    font-size:22px; letter-spacing:.16em; color:rgba(244,241,234,.55); font-weight:400;
  }
</style></head><body>
  <div class="glow"></div>
  <div class="frame"></div>
...
(async () => {
  ...
  for (const card of CARDS) {
    ...
    const file = path.join(OUT_DIR, `${card.slug}.png`);
    await page.screenshot({ path: file });
    const kb = Math.round(fs.statSync(file).size / 1024);
    console.log(`  ${card.slug}.png  ${kb}KB`);
  }
